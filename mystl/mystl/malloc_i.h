...
void* malloc_i::allocate(const size_t _Size) {
	void* result = malloc(_Size);
	if (nullptr == result) { result = oom_malloc(_Size); }
	return result;
}
void malloc_i::deallocate(void* _Block, size_t _Size) {
	free(_Block);
}
void* malloc_i::reallocate(void* _Block, size_t _old_Size, size_t _new_Size) {
	void* result = realloc(_Block, _new_Size);
	if (nullptr == result) { result = oom_realloc(_Block, _new_Size); }
	return result;
}
void(*malloc_i::set_oom_malloc_handler(void(*_new_handler)()))(){
